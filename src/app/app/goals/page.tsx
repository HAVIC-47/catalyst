      ...
      <div className="card mb-6 flex flex-wrap items-center gap-2 p-4">
        ...
        <div className="flex items-center gap-2 rounded-xl border border-line bg-white/[0.02] px-3 focus-within:border-amber/50">
          ...
          <input
            ...
            className="min-h-[44px] w-28 bg-transparent font-mono text-sm text-white outline-none"
          />
        </div>
        <input
          type="date"
            ...
            <div key={g.id} className="card p-5">
              ...
              <div className="h-2.5 overflow-hidden rounded-full bg-white/[0.06]">
                <div
                  ...
                  style={{ width: `${pct}%`, backgroundColor: done ? "#34D399" : "#22D3EE" }}
                />
              </div>
              <div className="mt-2 flex items-center justify-between font-mono text-sm">
                <span className="text-white/70">{formatCurrency(g.savedAmount)}</span>
                <span className="text-white/40">/ {formatCurrency(g.targetAmount)}</span>
              </div>
...
function AddSaving({ onAdd }: { onAdd: (amt: number) => void }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="flex items-center gap-1 rounded-lg border border-line bg-white/[0.02] px-2 focus-within:border-amber/50">
        <span className="font-mono text-xs text-white/40">{TAKA}</span>
        <input
          inputMode="decimal"
          value={v}
          ...
          className="min-h-[36px] w-20 bg-transparent font-mono text-xs text-white outline-none"
        />
      </div>
      <button
        onClick={() => {
        ...
        className="hairline flex h-9 w-9 cursor-pointer items-center justify-center rounded-lg text-income transition-colors hover:bg-white/5"
